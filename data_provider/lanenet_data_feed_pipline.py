...
class LaneNetDataFeeder(object):
    ...
    def __init__(self, flags='train'):
        ...
        self.tfrecords_file_paths = ops.join(self._tfrecords_dir, 'tusimple_{:s}.tfrecords'.format(self._dataset_flags))
        assert ops.exists(self.tfrecords_file_paths), '{:s} not exist'.format(self.tfrecords_file_paths)

        with tf.device('/cpu:0'):

            # TFRecordDataset opens a binary file and reads one record at a time.
            # `tfrecords_file_paths` could also be a list of filenames, which will be read in order.
            self.dataset = tf.data.TFRecordDataset(self.tfrecords_file_paths)
            ...
            self.dataset = self.dataset.shuffle(buffer_size=512)
            # # repeat num epochs
    ...
    def __len__(self):
        ...
        sample_counts = 0
        tfrecords = tf.data.TFRecordDataset(self.tfrecords_file_paths)
        sample_counts += sum(1 for _ in tfrecords)
        if self._dataset_flags == 'train':
    ...
    def next_batch(self, batch_size):
        ...
        self.dataset = self.dataset.prefetch(buffer_size=128)
